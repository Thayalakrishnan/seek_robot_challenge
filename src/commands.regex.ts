...
export class CommandRegexRegistry {
  ...
  constructor() {
    ...
    this.register("RIGHT", new RightCommand);
    this.register("LEFT", new LeftCommand);
    this.register("REPORT", new ReportCommand);
    this.pattern = this.buildPattern()
  }
  
  private buildAndWrapPattern(pattern: string): RegExp {
    const rpattern = `^(?<command>(${pattern}))(?<args> .*)?`;
    return new RegExp(rpattern)
  }
  
  private buildPattern(): RegExp {
    const commandKeys = [...this.commandDict.keys()];
    const commPatterns: string[] = [];
    commandKeys.forEach((key:string)=> {
        commPatterns.push(`(${key})`);
    })
    const joinedCommPatterns = commPatterns.join("|");
    return this.buildAndWrapPattern(joinedCommPatterns);
  }
  
  public register(commandName: string, commandClass: BaseCommand): void {
    this.commandDict.set(commandName, commandClass);
  }
  ...
  public getNullCommand(name = "NULL"): BaseCommand {
    return this.commandDict.get(name) ?? this.nullCommand
  }
}
